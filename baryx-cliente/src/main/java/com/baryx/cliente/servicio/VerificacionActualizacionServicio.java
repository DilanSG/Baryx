...
public class VerificacionActualizacionServicio {
    ...
    private static final String API_URL = System.getProperty(
            "baryx.api.url",
            "https://baryxweb.onrender.com"
    );

    private static final Path DIRECTORIO_UPDATES = Paths.get(
            System.getProperty("user.home"), ".baryx", "updates");
    ...
    private String extraerNombreArchivo(String urlDescarga, String version) {
        ...
        String plataforma = detectarPlataforma();
        return switch (plataforma) {
            case "windows" -> "Baryx-" + version + ".exe";
            case "macos" -> "Baryx-" + version + ".dmg";
            default -> "baryx_" + version + "_amd64.deb";
        };
    }

    /** Resuelve la ruta del ejecutable de Baryx según la plataforma actual. */
    private String resolverEjecutable() {
        String plataforma = detectarPlataforma();
        return switch (plataforma) {
            case "linux" -> "/opt/baryx/cliente/bin/Baryx";
            case "windows" -> resolverEjecutableWindows();
            case "macos" -> "/Applications/Baryx.app/Contents/MacOS/Baryx";
            default -> null;
        };
    }

    /** Busca el ejecutable de Baryx en las rutas estándar de Windows. */
    private String resolverEjecutableWindows() {
        // Ruta estándar de Inno Setup: Program Files\Baryx\Cliente\Baryx.exe
        String programFiles = System.getenv("ProgramFiles");
        if (programFiles != null) {
            Path candidato = Path.of(programFiles, "Baryx", "Cliente", "Baryx.exe");
            if (Files.exists(candidato)) return candidato.toString();
        }
        // Fallback: LocalAppData (instalación por usuario)
        String localAppData = System.getenv("LOCALAPPDATA");
        if (localAppData != null) {
            Path candidato = Path.of(localAppData, "Baryx", "Cliente", "Baryx.exe");
            if (Files.exists(candidato)) return candidato.toString();
        }
        // Último recurso: junto al JAR ejecutándose
        Path dirActual = Path.of(System.getProperty("user.dir"));
        Path candidato = dirActual.resolve("Baryx.exe");
        if (Files.exists(candidato)) return candidato.toString();
        return null;
    }
